	...
	struct wpa_supplicant *wpa_s, char *addr)
{
	...
	/* ignore discovery requests while in auto-mode */
	...
	struct wpa_supplicant *wpa_s, char *addr)
{
	...
	if (wpa_s->conf->tdls_auto_enabled)
		return wpas_tdls_auto_start(wpa_s, peer);
	...
	struct wpa_supplicant *wpa_s, char *addr)
{
	...
	if (os_strcmp(addr, "*") == 0) {
		...
		if (wpa_s->conf->tdls_auto_enabled)
			wpas_tdls_auto_remove_peers(wpa_s, 1);
		else
	...
	if (wpa_s->conf->tdls_auto_enabled) {
		wpas_tdls_auto_stop(wpa_s, peer);
		return 0;
	}
...
p2p_parse_asp_provision_cmd(struct wpa_supplicant *wpa_s, const char *cmd)
{
	...
	if (pos) {
		if (sscanf(pos + 12, "%llx", &val) != 1 || val > 0xffffULL ||
		    (~cpt_mask & (val & 0xFF)))
			goto invalid_args;

	} else {
		/* P2PS defines only CPT field (byte 0) of feature capability */
		val = cpt_mask;
	}

	p2ps_prov->feature_cap[0] = val & 0xFF;
	p2ps_prov->feature_cap[1] = (val >> 8) & 0xFF;
...
static int wpa_supplicant_signal_poll(struct wpa_supplicant *wpa_s, char *buf,
				      size_t buflen)
{
	struct wpa_chan_info si;
	int ret;
	char *pos, *end;

	ret = wpa_drv_channel_poll(wpa_s, &si);
	if (ret)
		return -1;
...
static void wpa_supplicant_ctrl_iface_flush(struct wpa_supplicant *wpa_s)
{
	...
	p2p_wpa_s->global->p2p_disabled &= ~WPA_P2P_MGMT_CTRL_DISABLED;
	...
	radio_remove_works(wpa_s, NULL, 1, 1);
...
static int wpas_global_ctrl_iface_set_tx_power(struct wpa_global *global,
					       int tx_power)
{
	...
	for (ifs = global->ifaces; ifs; ifs = ifs->next) {
		...
		if (ifs->current_ssid &&
		    (ifs->current_ssid->mode == WPAS_MODE_P2P_GO ||
		     ifs->current_ssid->mode == WPAS_MODE_AP) &&
		    ifs->ap_iface) {
				ifs->ap_iface->ext_tx_power = tx_power;
				hostapd_set_tx_power(ifs->ap_iface);
		} else {
			wpa_drv_set_tx_power(ifs, tx_power);
		}
	}
...
static int wpas_global_ctrl_iface_set(struct wpa_global *global, char *cmd)
{
	...
	*value++ = '\0';
	...
	if (os_strcasecmp(cmd, "tx_power") == 0)
		return wpas_global_ctrl_iface_set_tx_power(global, atoi(value));
...
static int wpas_global_ctrl_iface_used_freqs(struct wpa_global *global,
					     char *buf, size_t buflen)
{
	char *pos, *end;
	int ret;
	...
	for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
		os_memset(&si, 0 , sizeof(struct wpa_chan_info));
		if (!wpa_s->current_ssid || wpa_s->wpa_state != WPA_COMPLETED ||
		    wpa_drv_channel_poll(wpa_s, &si))
			continue;

		ieee80211_freq_to_chan(si.frequency, &channel);
		ret = os_snprintf(pos, end - pos,
				  "%s: channel=%d (%d MHz), width=%s, cf1=%d, cf2=%d\n",
				  wpa_s->ifname, channel, si.frequency,
				  channel_width_to_string(si.chanwidth),
				  si.center_frq1, si.center_frq2);
		if (ret < 0 || ret >= end - pos)
			return pos - buf;
						...
						char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		reply_len = wpas_global_ctrl_iface_status(global, reply,
							  reply_size);
	} else if (os_strcmp(buf, "USED_FREQS") == 0) {
		reply_len = wpas_global_ctrl_iface_used_freqs(global, reply,
							      reply_size);
