...
static inline int wpa_drv_set_indoor(struct wpa_supplicant *wpa_s,
				      unsigned int indoor)
{
	if (wpa_s->driver->set_indoor)
		return wpa_s->driver->set_indoor(wpa_s->drv_priv, indoor);
	return 0;
}
...
static inline int wpa_drv_channel_poll(struct wpa_supplicant *wpa_s,
				       struct wpa_chan_info *si)
{
	if (wpa_s->driver->channel_poll)
		return wpa_s->driver->channel_poll(wpa_s->drv_priv, si);
	return -1;
}
...
static inline int wpa_drv_read_sta_data(struct wpa_supplicant *wpa_s,
					struct hostap_sta_driver_data *sta,
					const u8 *addr)
{
	if (wpa_s->driver->read_sta_data)
...
static inline int wpa_drv_set_tx_power(struct wpa_supplicant *wpa_s,
				       int tx_power)
{
	if (!wpa_s->driver->set_tx_power)
		return -1;
	return wpa_s->driver->set_tx_power(wpa_s->drv_priv, tx_power);
}
