...
static inline int hostapd_drv_set_tx_power(struct hostapd_data *hapd,
					   int tx_power)
{
	if (hapd->driver == NULL || hapd->driver->set_tx_power == NULL)
		return -1;
	return hapd->driver->set_tx_power(hapd->drv_priv, tx_power);
}
