			      ...
			      struct nlattr *tb[])
{
	...
	res = nl80211_channel_poll(drv->first_bss, &ed.signal_change);
	if (res == 0) {
		wpa_printf(MSG_DEBUG,
			   "nl80211: Signal: %d dBm  txrate: %d noise: %d dBm",
				     ...
				     int cmd, struct nlattr *tb[])
{
	...
	/*
	 * only process a private regulatory event if we're a self-managed
	 * device and the change is for us
	 */
	if (cmd == NL80211_CMD_WIPHY_REG_CHANGE &&
	    (!drv->self_managed_reg || !tb[NL80211_ATTR_WIPHY] ||
	     nla_get_u32(tb[NL80211_ATTR_WIPHY]) != drv->wiphy_idx))
		return;

	/* self-managed devices don't care about global changes */
	if (cmd == NL80211_CMD_REG_CHANGE && drv->self_managed_reg)
		return;
				 ...
				 struct nlattr **tb)
{
	...
	case NL80211_CMD_REG_CHANGE:
		nl80211_reg_change_event(drv, cmd, tb);
		break;
