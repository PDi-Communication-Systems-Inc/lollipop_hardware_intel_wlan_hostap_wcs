...
{
	drv_free_hw_features(wpa_s->hw.modes, wpa_s->hw.num_modes);
	wpa_s->hw.modes = NULL;
...
#ifdef CONFIG_HS20
		/*
		 * It is assumed that the filters are cleared on disconnection
		 */
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	...
	if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_SME) &&
		...
		if (radio_work_pending(wpa_s, "scan")) {
			...
			radio_remove_works(wpa_s, "scan", 0, 1);
		}
...
/*
 * This function removes radio works running on the provided interface's radio.
 ...
 * @remove_started: 1 to allow removing already started work, 0 to remove only
 * pending works.
 */
void radio_remove_works(struct wpa_supplicant *wpa_s,
			const char *type, int remove_all, int remove_started)
{
	struct wpa_radio_work *work, *tmp;
	struct wpa_radio *radio = wpa_s->radio;
	int started_removed = 0;
	...
	dl_list_for_each_safe(work, tmp, &radio->work, struct wpa_radio_work,
		...
		if (!remove_started && work->started) {
			wpa_dbg(wpa_s, MSG_DEBUG,
				"Skip removing already started radio work '%s'@%p",
				work->type, work);
			continue;
		}

		started_removed |= work->started;
				     ...
				     struct wpa_interface *iface)
{
	...
	if ((!iface->p2p_mgmt ||
	    ...
	    (wpa_tdls_init(wpa_s->wpa) || wpas_tdls_auto_init(wpa_s)))
		return -1;
#endif /* CONFIG_TDLS */
						 ...
						 struct wpa_supplicant *parent)
{
	...
	if (global->ext_tx_power && !wpa_s->p2p_mgmt)
		wpa_drv_set_tx_power(wpa_s, global->ext_tx_power);
...
int wpas_setband(struct wpa_supplicant *wpa_s, int band)
{
	...
	if (wpa_s->disconnected || wpa_s->auto_reconnect_disabled)
		return 0;
...
wpas_get_hw_channel_data(struct wpa_supplicant *wpa_s,
			 enum hostapd_hw_mode mode, u8 channel)
{
	int i, j;
	...
	for (i = 0; i < wpa_s->hw.num_modes; i++) {
		if (wpa_s->hw.modes[i].mode != mode)
			continue;

		hw_mode = &wpa_s->hw.modes[i];
		for (j = 0; j < hw_mode->num_channels; j++)
			if (hw_mode->channels[j].chan == channel)
				return &hw_mode->channels[j];
	}
	return NULL;
}
