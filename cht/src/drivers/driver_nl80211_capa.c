				     ...
				     struct nlattr *tb)
{
	...
	if (flags & NL80211_FEATURE_VIF_TXPOWER)
		info->support_vif_txpower = 1;
...
static int wiphy_info_handler(struct nl_msg *msg, void *arg)
{
	...
	if (tb[NL80211_ATTR_WIPHY_SELF_MANAGED_REG]) {
		wpa_printf(MSG_DEBUG, "nl80211: Self-managed regulatory");
		drv->self_managed_reg = 1;
	}
...
int wpa_driver_nl80211_capa(struct wpa_driver_nl80211_data *drv)
{
	...
	drv->support_vif_txpower = info.support_vif_txpower;
...
static void nl80211_reg_rule_vht(struct nlattr *tb[],
				 struct phy_info_arg *results)
{
	u32 start, end, max_bw, flags;
	u16 m;

	if (tb[NL80211_ATTR_FREQ_RANGE_START] == NULL ||
	    tb[NL80211_ATTR_FREQ_RANGE_END] == NULL ||
	    tb[NL80211_ATTR_FREQ_RANGE_MAX_BW] == NULL ||
	    tb[NL80211_ATTR_REG_RULE_FLAGS] == NULL)
		return;
...
static int nl80211_get_reg(struct nl_msg *msg, void *arg)
{
	...
	nla_for_each_nested(nl_rule, tb_msg[NL80211_ATTR_REG_RULES], rem_rule)
	{
		...
		wpa_printf(MSG_DEBUG,
			   "nl80211: %u-%u @ %u MHz %u mBm%s%s%s%s%s%s%s%s%s%s",
	...
	if (send_and_recv_msgs(drv, msg, phy_info_handler, &result) == 0) {
		if (result.error) {
			wpa_printf(MSG_ERROR,
				   "nl80211: error while get hw_feature_data");

			drv_free_hw_features(result.modes, *num_modes);
			*num_modes = 0;
			*flags = 0;
			return NULL;
		}
