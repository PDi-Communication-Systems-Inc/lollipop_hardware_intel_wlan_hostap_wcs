...
#define HOSTAPD_CHAN_NO_80MHZ 0x00040000
#define HOSTAPD_CHAN_NO_160MHZ 0x00080000

/* Filter gratuitous ARP */
#define WPA_DATA_FRAME_FILTER_FLAG_ARP BIT(0)
/* Filter unsolicited Neighbor Advertisment */
#define WPA_DATA_FRAME_FILTER_FLAG_NA BIT(1)
/* Filter frames encrypted using the GTK */
...
#define WPA_DRIVER_CAPA_KEY_MGMT_WAPI_CERT	0x00000400
...
/**
 * struct wpa_chan_info - Information about channel signal quality
 */
struct wpa_chan_info {
	u32 frequency;
	/* This information is only relevant for station interfaces */
	int above_threshold;
	int current_signal;
	int avg_signal;
...
struct wpa_driver_ops {
	 ...
	 * @freq: freq to send the frame on, or 0 to let the driver decide
	 * Returns: 0 on success, -1 on failure
	 */
	int (*send_mlme)(void *priv, const u8 *data, size_t data_len,
			 int noack, u16 *csa_offs, size_t csa_offs_len,
			 unsigned int freq);
	...
	/**
	 * set_indoor - Set indoor regulatory indication
	 * @priv: Private driver interface data
	 * @indoor: indication if operating in an indoor environment
	 * Returns: 0 on success, -1 on failure
	 *
	 * This function can be used to indicate to the driver that the
	 * device is operating an indoor environment or not.
	 */
	int (*set_indoor)(void *priv, unsigned int indoor);
	...
	/**
	 * set_tx_power - Limit the tx power for the current interface
	 * @priv: Private driver interface data
	 * @tx_power: Maximum transmit power in dBm, or 0 in case the
	 *	tx power should be controlled by the driver.
	 * Returns: 0 on success or -1 on failure
	 */
	int (*set_tx_power)(void *priv, int tx_power);

	/**
	 * configure_data_frame_filters - Request to configure frame filters
	 * @priv: Private driver interface data
	 * @filter_flags: The type of frames to filter.
	 * Returns: 0 on success or -1 on failure
	 */
	...
	/**
	 * channel_poll - Get current connection information
	 * @priv: Private driver interface data
	 * @chan_info: Connection info structure
	 */
	int (*channel_poll)(void *priv, struct wpa_chan_info *chan_info);
...
union wpa_event_data {
	 ...
	 */
	struct wpa_chan_info signal_change;
...
static inline void drv_free_hw_features(struct hostapd_hw_modes *hw_features,
