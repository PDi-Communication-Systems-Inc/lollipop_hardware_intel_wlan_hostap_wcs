...
static int get_link_signal(struct nl_msg *msg, void *arg)
{
	...
	struct wpa_chan_info *sig_change = arg;
...
int nl80211_get_link_noise(struct wpa_driver_nl80211_data *drv,
			   struct wpa_chan_info *sig_change)
{
	struct nl_msg *msg;
...
static int wpa_driver_nl80211_set_country(void *priv, const char *alpha2_arg)
{
	...
	/*
	 * we don't support set-country when regulatory is managed entirely by
	 * the card.
	 */
	if (drv->self_managed_reg) {
		wpa_printf(MSG_DEBUG,
			   "nl80211: ignoring country update on self-managed");
		return 0;
	}
...
static int
wpa_driver_nl80211_set_indoor(void *priv, unsigned int indoor)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
	...
	if (indoor)
		NLA_PUT_FLAG(msg, NL80211_ATTR_REG_INDOOR);

	NLA_PUT_FLAG(msg, NL80211_ATTR_SOCKET_OWNER);

	wpa_printf(MSG_DEBUG, "nl80211: set regulatory indoor to %d", indoor);
	return send_and_recv_msgs(drv, msg, NULL, NULL);
...
int nl80211_set_p2pdev(struct i802_bss *bss, int start)
{
	struct nl_msg *msg;
	int ret;
...
static int nl80211_get_channel_info(struct wpa_driver_nl80211_data *drv,
				    struct wpa_chan_info *sig)
{
	struct nl_msg *msg;

	msg = nl80211_drv_msg(drv, 0, NL80211_CMD_GET_INTERFACE);
	return send_and_recv_msgs(drv, msg, get_channel_info, sig);
}


int nl80211_channel_poll(void *priv, struct wpa_chan_info *si)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	int res;
	...
	/*
	 * Link signal and noise information is only relevant for station
	 * interfaces.
	 */
	if (is_sta_interface(drv->nlmode)) {
		...
		res = nl80211_get_link_noise(drv, si);
		if (res != 0)
			return res;
	...
	return nl80211_get_channel_info(drv, si);
}
...
int nl80211_set_power_save(struct i802_bss *bss, int enabled)
{
	struct nl_msg *msg;
...
static int nl80211_set_tx_power(void *priv, int tx_power)
{
	struct i802_bss *bss = priv;
	struct nl_msg *msg;
	enum nl80211_iftype nlmode;

	nlmode = bss->drv->nlmode;
	if ((!is_ap_interface(nlmode) && !is_sta_interface(nlmode)) ||
	      !bss->drv->support_vif_txpower)
		return -EOPNOTSUPP;
	...
	nl80211_cmd(bss->drv, msg, 0, NL80211_CMD_SET_WIPHY);
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, bss->ifindex);

	if (!tx_power) {
		wpa_printf(MSG_DEBUG, "nl80211: set tx power=automatic");
		NLA_PUT_U32(msg, NL80211_ATTR_WIPHY_TX_POWER_SETTING,
			NL80211_TX_POWER_AUTOMATIC);
	} else {
		wpa_printf(MSG_DEBUG, "nl80211: set tx power=%d",
			tx_power);
		NLA_PUT_U32(msg, NL80211_ATTR_WIPHY_TX_POWER_SETTING,
...
static int nl80211_configure_data_frame_filters(void *priv, u32 filter_flags)
{
	...
	wpa_printf(MSG_DEBUG,
		   "nl80211: Configuring data frame filtering for flags=0x%x\n",
		   filter_flags);

	if (filter_flags & WPA_DATA_FRAME_FILTER_FLAG_ARP &&
	    filter_flags & WPA_DATA_FRAME_FILTER_FLAG_NA)
		NLA_PUT_FLAG(msg, IWL_MVM_VENDOR_FILTER_ARP_NA);
	if (filter_flags & WPA_DATA_FRAME_FILTER_FLAG_GTK)
		NLA_PUT_FLAG(msg, IWL_MVM_VENDOR_FILTER_GTK);
	...
	wpa_printf(MSG_DEBUG,
		   "%s: Sending filtering vendor cmd %s (vendor = %d)\n",
		   __func__, ret ? "Failed" : "Succeeded", INTEL_OUI);
...
int nl80211_set_country(void *priv, const char *alpha2_arg)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
	...
	*/
	...
	NLA_PUT_STRING(msg, IWL_MVM_VENDOR_ATTR_COUNTRY, alpha2_arg);
...
const struct wpa_driver_ops wpa_driver_nl80211_ops = {
	...
	.set_indoor = wpa_driver_nl80211_set_indoor,
	...
	.channel_poll = nl80211_channel_poll,
	...
	.set_tx_power = nl80211_set_tx_power,
