	     ...
	     u8 seek_count, const char **seek, int freq)
{
	...
	if (seek_count == 0 || !seek) {
		...
		for (cnt = 0, i = 0; i < seek_count; i++) {
			...
			os_memcpy(&p2p->p2ps_seek_hash[cnt++ * P2PS_HASH_LEN],
				  buf, P2PS_HASH_LEN);
		}

		p2p->p2ps_seek_count = cnt;
		p2p->p2ps_seek = 1;
	} else {
		p2p->p2ps_seek_count = 0;
		p2p->p2ps_seek = 1;
	...
	case P2P_FIND_START_WITH_FULL:
		if (freq > 0) {
			scan_type = P2P_SCAN_SPECIFIC;
			break;
		}
		/* fall through */
	case P2P_FIND_PROGRESSIVE:
		scan_type = P2P_SCAN_FULL;
		break;
	case P2P_FIND_ONLY_SOCIAL:
		scan_type = P2P_SCAN_SOCIAL;
		break;
	default:
		return -1;
	}

	res = p2p->cfg->p2p_scan(p2p->cfg->cb_ctx, scan_type,
				 freq, p2p->num_req_dev_types,
	...
	if (res == 0) {
		...
		p2p_dbg(p2p, "Failed to start p2p_scan - another p2p_scan already in progress");
...
void p2p_stop_find_for_freq(struct p2p_data *p2p, int freq)
{
	...
	if (state == P2P_SEARCH || state == P2P_SD_DURING_FIND)
...
static int p2p_service_find_asp(struct p2p_data *p2p, const u8 *hash)
{
	...
	/* P2PS wildcard always matches if we have actual services */
	...
	while (adv_data) {
		...
		if (os_memcmp(hash, adv_data->hash, P2PS_HASH_LEN) == 0 ||
		    (os_memcmp(hash, p2p->wildcard_hash_wsb_dev,
			       P2PS_HASH_LEN) == 0 &&
		     os_memcmp(P2PS_WILD_HASH_WSB_DEV_STR, adv_data->svc_name,
			       sizeof(P2PS_WILD_HASH_WSB_DEV_STR) - 1) == 0)) {
			p2p_dbg(p2p, "ASP hash match: %s", adv_data->svc_name);
			return 1;
		}
...
struct p2p_data * p2p_init(const struct p2p_config *cfg)
{
	...
	p2ps_gen_hash(p2p, P2PS_WILD_HASH_WSB_DEV_STR,
		      p2p->wildcard_hash_wsb_dev);
...
void p2p_continue_find(struct p2p_data *p2p)
{
	...
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		...
		remaining_usec = 0;
		if (p2p_pre_find_operation(p2p, dev, &remaining_usec) > 0) {
...
{
	struct p2p_channels tmp;

	os_memcpy(&tmp, res, sizeof(tmp));
	p2p_channels_intersect(&tmp, b, res);
}
